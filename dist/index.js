...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    switch (name) {
        ...
        case "get_class": {
            ...
            const ssrSafe = args?.ssrSafe ?? false;
            ...
            if (!pattern) {
                ...
                const similar = patterns.filter(p => p.id.includes(className) || className.includes(p.id) ||
                    p.name.toLowerCase().includes(className.toLowerCase())).slice(0, 3);
                return {
                    content: [
                        {
                            type: "text",
        ...
        case "list_classes": {
            ...
            const ssrSafeOnly = args?.ssrSafeOnly ?? false;
