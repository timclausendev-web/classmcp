 ...
 */
async function tryReadPackageJson(projectPath) {
 ...
 */
export async function reloadConfig(configPath, projectPath) {
    if (!configPath) {
    ...
    if (configPath.includes('#classmcp')) {
        ...
        if (pkgResult) {
            const validation = validateUserConfig(pkgResult.data);
            return {
                config: validation.valid ? pkgResult.data : DEFAULT_CONFIG,
                configPath: pkgResult.path,
                validation,
                found: true,
            };
    ...
    const validation = validateUserConfig(result.data);
    return {
        config: validation.valid ? result.data : DEFAULT_CONFIG,
        configPath,
        validation,
        found: true,
    };
