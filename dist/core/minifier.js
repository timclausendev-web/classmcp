 ...
 */
export function hashClasses(classes) {
    let hash = 0;
 ...
 */
export function generateMinifiedName(counter) {
 ...
 */
export function createMinificationMap() {
    return {
 ...
 */
export function minifyClass(map, semanticName, classes) {
 ...
 */
export function getMinified(map, semanticName) {
    return map.nameToMinified.get(semanticName);
}
/**
 * Get full data for a minified name
 */
export function getFromMinified(map, minified) {
    return map.minifiedToData.get(minified);
}
/**
 * Generate CSS from minification map
 */
export function generateMinifiedCSS(map, options = { framework: 'tailwind', includeComments: false }) {
    let css = `/* Generated by classmcp - Ultra-minified CSS */\n\n`;
    const sorted = Array.from(map.minifiedToData.values()).sort((a, b) => a.minified.localeCompare(b.minified));
 ...
 */
export function calculateSavings(map) {
    let totalOriginal = 0;
    let totalMinified = 0;
    for (const [_, data] of map.minifiedToData) {
