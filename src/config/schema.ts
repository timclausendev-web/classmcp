...
import type { CustomPatternInput, CustomStateVariant, UserConfig } from '../types.js';

// Valid framework IDs
const VALID_FRAMEWORKS = ['tailwind', 'bootstrap', 'unocss', 'tachyons'] as const;
 ...
 */
function validatePatternId(id: unknown, index: number): ValidationError | null {
  if (typeof id !== 'string') {
 ...
 */
function validateClasses(classes: unknown, index: number): ValidationError | null {
  ...
  if (typeof classes === 'object' && classes !== null) {
    const stateVariant = classes as CustomStateVariant;
    ...
    const stateFields = ['hover', 'focus', 'active', 'disabled'] as const;
 ...
 */
function validatePattern(pattern: unknown, index: number): ValidationError[] {
  ...
  if (p.frameworks !== undefined) {
    if (!Array.isArray(p.frameworks)) {
      ...
      for (let i = 0; i < p.frameworks.length; i++) {
        if (!VALID_FRAMEWORKS.includes(p.frameworks[i] as typeof VALID_FRAMEWORKS[number])) {
          errors.push({
  ...
  if (p.ssr !== undefined) {
    if (typeof p.ssr !== 'object' || p.ssr === null) {
      ...
      const ssr = p.ssr as Record<string, unknown>;
...
export function validateUserConfig(config: unknown): ValidationResult {
  ...
  if (c.defaultFramework !== undefined) {
    if (!VALID_FRAMEWORKS.includes(c.defaultFramework as typeof VALID_FRAMEWORKS[number])) {
      errors.push({
  ...
  if (c.customPatterns !== undefined) {
    if (!Array.isArray(c.customPatterns)) {
      ...
      for (let i = 0; i < c.customPatterns.length; i++) {
        ...
        const pattern = c.customPatterns[i] as CustomPatternInput;
        if (pattern?.id) {
